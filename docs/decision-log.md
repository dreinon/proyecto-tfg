...
| DEC-SCI-05 | 2026-08-31 | Student | decided | Close domain fine-tuning as NO-GO for this milestone | SMB is evaluation-only and no independent leakage-safe training/validation corpus is in scope; the pretrained core answers the controlled question while adaptation would jeopardize analysis, tutor review, and deposit quality | tracked-ref:.planning/phases/03-pretrained-methods-and-adaptation-decision/03-VERIFICATION.md | METH-08; no training run; bicubic, EDSR, and SwinIR comparison retained unchanged |
| DEC-SCI-06 | 2026-08-31 | Student | decided | Use EDSR as the default fidelity/cost option and keep SwinIR condition-dependent | Both learned methods beat bicubic on the primary metrics in all six conditions, but SwinIR is 5.2--8.5 times slower than EDSR and does not win every direct paired comparison; qualitative cases prevent semantic-safety claims for either network | tracked-ref:artifacts/kaggle/phase3-smb-analysis-v2/final/analysis-manifest.json; tracked-ref:memoria/chapters/07-validacion-y-aplicacion-profesional.tex | Professional guidance for controlled SMB v2 only; original retention and human verification mandatory |
| DEC-SCI-07 | 2026-09-01 | Student | decided | Supersede DEC-SCI-05 for one bounded secondary EDSR adaptation study | A separate corpus name is not required when source works are assigned prospectively to disjoint roles, the primary 64-work benchmark stays excluded, checkpoint selection uses validation only, and a fresh test remains untouched | tracked-ref:docs/smb-edsr-finetuning-v1.md; tracked-ref:docs/smb-edsr-finetuning-v1-results.md | DEC-SCI-05 remains the historical primary-core gate; EDSR x2/x4 only; within-SMB matched-degradation claims |

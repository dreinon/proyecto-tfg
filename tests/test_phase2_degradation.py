...
VISUAL_FIXTURE_MANIFEST_V3_PATH = (
...
def _copy_preview_project(destination: Path) -> None:
    tracked_inputs = (
        ...
        Path("tests/fixtures/phase2/visual-fixture-manifest-v3.yaml"),
...
def test_dense_roi_manifest_v3_reuses_source_bytes_and_is_fragment_first() -> None:
    manifest_v2 = _yaml(VISUAL_FIXTURE_MANIFEST_V2_PATH)
    manifest_v3 = _yaml(VISUAL_FIXTURE_MANIFEST_V3_PATH)

    assert manifest_v3["manifest_id"] == "phase2-engraved-visual-fixtures-v3"
    assert manifest_v3["renderer"] == manifest_v2["renderer"]
    assert manifest_v3["required_semantics"] == manifest_v2["required_semantics"]
    for current, prior in zip(manifest_v3["items"], manifest_v2["items"], strict=True):
        assert {key: current[key] for key in current if key not in {"roi"}} == {
            key: prior[key] for key in prior if key not in {"roi"}
        }
    expected_rois = {
    ...
    items = {item["item_id"]: item for item in manifest_v3["items"]}
    assert items["review-work-03-excerpt-01"]["roi"] == expected_rois["review-work-03-excerpt-01"]
    assert items["review-work-04-excerpt-01"]["roi"] == expected_rois["review-work-04-excerpt-01"]
    assert [
        (panel["item_id"], panel["condition_id"]) for panel in manifest_v3["review_membership"]
    ] == [
        *(("review-work-03-excerpt-01", condition) for condition in EXPECTED_CELLS),
        *(("review-work-04-excerpt-01", condition) for condition in EXPECTED_CELLS),
    ]
    assert all(
        panel["source_group_id"] == panel["item_id"].removesuffix("-excerpt-01")
        for panel in manifest_v3["review_membership"]
    )
...
def test_candidate_v5_preview_has_dense_fragment_first_membership_and_exact_panels(
    ...
    assert [panel["item_id"] for panel in membership["panels"]] == [
    ...
    assert all(panel["roi"] == expected_rois[panel["item_id"]] for panel in membership["panels"])
...
def test_candidate_v5_preview_summary_groups_each_fragment_before_the_next(
    ...
    assert presentation == [
        ...
        ("image", "panel-02-x2-moderate.png"),
        ("markdown", "#### strong (`x2-strong`)"),
        ("image", "panel-03-x2-strong.png"),
        ("markdown", "### x4"),
        ("markdown", "#### clean (`x4-clean`)"),
        ("image", "panel-04-x4-clean.png"),
        ("markdown", "#### moderate (`x4-moderate`)"),
        ("image", "panel-05-x4-moderate.png"),
        ("markdown", "#### strong (`x4-strong`)"),
        ("image", "panel-06-x4-strong.png"),
        ("markdown", "## Fragment 2 — `review-work-04-excerpt-01`"),
        ("markdown", "### x2"),
        ("markdown", "#### clean (`x2-clean`)"),
        ("image", "panel-07-x2-clean.png"),
        ("markdown", "#### moderate (`x2-moderate`)"),
        ("image", "panel-08-x2-moderate.png"),
        ("markdown", "#### strong (`x2-strong`)"),
        ("image", "panel-09-x2-strong.png"),
        ("markdown", "### x4"),
        ("markdown", "#### clean (`x4-clean`)"),
        ("image", "panel-10-x4-clean.png"),
        ("markdown", "#### moderate (`x4-moderate`)"),
        ("image", "panel-11-x4-moderate.png"),
        ("markdown", "#### strong (`x4-strong`)"),
...
def test_candidate_v5_preview_uses_only_semantically_complete_engraved_review_fixtures(
    ...
    assert membership["selection_policy"] == "paired-dense-fragment-first-v3"
...
def test_preview_publication_candidate_v5_has_exact_inventory_and_is_idempotent(
    ...
    expected = {
        ...
        *(
            ...
            for index, condition in enumerate([*EXPECTED_CELLS, *EXPECTED_CELLS], start=1)
        ),
